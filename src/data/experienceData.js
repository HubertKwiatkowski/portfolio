const experienceData = [
  {
    companyName: 'Field Geospatial',
    position: 'modeler and scanner operator',
    city: 'Oslo',
    country: 'Norway',
    startDate: '11.2023',
    finishDate: 'now',
    responsibilities: [
      '3D data acquisition and point cloud processing, preparing Revit models of scanned objects in revit software',
    ],
  },
  {
    companyName: 'Leap3D',
    position: 'modeler and scanner operator',
    city: 'Rijen',
    country: 'Netherlands',
    startDate: '11.2018',
    finishDate: '11.2022',
    responsibilities: [
      'direct contact with clients, 3D data acquisition and point cloud processing, preparing Revit models of scanned objects in revit software',
    ],
  },
  {
    companyName: 'Iterator',
    position: 'Junior Python/Django dev',
    city: 'Rotterdam',
    country: 'Netherlands',
    startDate: '01.2022',
    finishDate: '06.2022',
    responsibilities: [
      'developing web applications under supervision of Senior Software Developer using Django and PostgreSQL',
    ],
  },
  {
    companyName: 'BD GROUP',
    position: 'Project Design Manager',
    city: 'Wroclaw',
    country: 'Poland',
    startDate: '12.2016',
    finishDate: '11.2018',
    responsibilities: [
      'managing a group of junior designers, cooperation with contractors during project realizations, obtaining building permits for designed buildings, Revit modelling, CAD drawing, BIM management, system design for building’s installation systems',
    ],
  },
  {
    companyName: 'INSTALL STUDIO',
    position: 'Senior assistant designer',
    city: 'Wroclaw',
    country: 'Poland',
    startDate: '11.2015',
    finishDate: '12.2016',
    responsibilities: [
      'projects coordination, mechanical calculation and design for building’s installations, consulting design solutions with specialist',
    ],
  },
  {
    companyName: 'JANURA',
    position: 'Senior assistant designer',
    city: 'Wroclaw',
    country: 'Poland',
    startDate: '08.2014',
    finishDate: '11.2015',
    responsibilities: [
      'designing of sanitation and water supply systems, rainwater retention systems',
    ],
  },
  {
    companyName: 'PUH MAMAS',
    position: 'Assistant designer',
    city: 'Wroclaw',
    country: 'Poland',
    startDate: '06.2011',
    finishDate: '07.2014',
    responsibilities: [
      'mechanical design for building installation and systems, work on a variety of projects: government facilities healthcare, industrial, commercial and education facilities',
    ],
  },
  {
    companyName: 'PPIS G. i J. Szumscy',
    position: 'Assistant designer',
    city: 'Bialystok',
    country: 'Poland',
    startDate: '10.2008',
    finishDate: '05.2011',
    responsibilities: [
      'work in a team setting with other team members and disciplines such as electrical, architectural and civil, designing building systems including HVAC, plumbing, piping and fire protection',
    ],
  },
];

export default experienceData;
